...
const tools = [
  {
    name: "health",
    description: "Check whether the configured Epixodo Lite PocketBase instance is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
  ...
  {
    name: "list_projects",
    description: "List Epixodo Lite projects ordered like the app.",
    inputSchema: {
      type: "object",
      properties: {
        filter: { type: "string" },
        perPage: { type: "number", minimum: 1, maximum: 500, default: 200 },
      },
      additionalProperties: false,
    },
  },
  {
    name: "list_tasks",
    description: "List Epixodo Lite tasks, optionally filtered by project id.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Check whether the PocketBase collections expected by Epixodo Lite exist.",
    inputSchema: {
      type: "object",
      properties: {},
      additionalProperties: false,
...
const toolHandlers = {
  ...
  async list_projects(args) {
    const filter = combineFilters([args.filter]);
    return toolHandlers.list_records({
      collection: "projects",
      fullList: true,
      perPage: numberOrDefault(args.perPage, 200),
      sort: "order,-created",
      ...(filter ? { filter } : {}),
    });
  },
  ...
  async list_tasks(args) {
    const filter = combineFilters([args.projectId ? `project = "${args.projectId}"` : "", args.filter]);
    return toolHandlers.list_records({
      collection: "tasks",
      fullList: true,
      perPage: numberOrDefault(args.perPage, 200),
  ...
  switch (message.method) {
    case "initialize":
      return {
        ...
        serverInfo: {
          name: "mcp-pocketbase-epixodo-lite",
          version: "0.1.0",
        },
      };
    ...
    case "resources/list":
      return {
        resources: [
          ...
          {
            uri: "pocketbase://epixodo-schema",
            name: "Epixodo Lite schema status",
            description: "Expected collection names and which ones are currently missing.",
            mimeType: "application/json",
          },
        ],
      };
    ...
    case "resources/read":
      ...
      if (message.params?.uri === "pocketbase://epixodo-schema") {
        return {
          contents: [
            {
              uri: "pocketbase://epixodo-schema",
              mimeType: "application/json",
              text: JSON.stringify(await toolHandlers.validate_epixodo_schema(), null, 2),
            },
          ],
        };
      }
...
function combineFilters(filters) {
  return filters.filter((filter) => typeof filter === "string" && filter.trim()).join(" && ");
}
